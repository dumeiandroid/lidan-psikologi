...
export async function onRequest(context) {
  ...
  try {
    switch (method) {
      case 'GET':
        return await getContacts(DB_PSIKOLOGI, tableName, corsHeaders);
      case 'POST':
        return await createContact(request, DB_PSIKOLOGI, tableName, corsHeaders);
      default:
...
async function getContacts(DB_PSIKOLOGI, tableName, corsHeaders) {
  ...
  try {
    ...
    const { results } = await DB_PSIKOLOGI.prepare(query).all();
...
async function createContact(request, DB_PSIKOLOGI, tableName, corsHeaders) {
  ...
  try {
    ...
    const result = await DB_PSIKOLOGI.prepare(query).bind(...values).run();
